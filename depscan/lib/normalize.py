...
def create_pkg_variations(pkg_dict):
    ...
    if vendor:
        ...
        if (
            ...
            if len(tmpA) > 1 and len(tmpA[1]) > 3 and tmpA[1] != name:
    ...
    if (
        ...
        and not name.startswith("go")
        and "golang" not in vendor
        and name not in ["net", "crypto", "http"]
    ):
        vendor_aliases.add("golang")
    if not purl.startswith("pkg:golang"):
        vendor_aliases.add(f"get{name}")
        vendor_aliases.add(f"{name}_project")
    ...
    if len(vendor_aliases):
        for vvar in list(vendor_aliases):
            pkg_list.extend(
                {**pkg_dict, "vendor": vvar, "name": nvar}
                for nvar in list(name_aliases)
            )

    else:
        pkg_list.extend({**pkg_dict, "name": nvar} for nvar in list(name_aliases))
    return pkg_list
...
def dealias_packages(project_type, pkg_list, pkg_aliases):
    ...
    for res in pkg_list:
        ...
        for k, v in pkg_aliases.items():
            if (
                full_pkg in v
                or f":{package_issue.affected_location.package}" in v
            ) and full_pkg != k:
...
def dedup(project_type, pkg_list, pkg_aliases):
    ...
    for res in pkg_list:
        ...
        if (
            package_type
            and package_type in KNOWN_PKG_TYPES
            and allowed_type
            and allowed_type != package_type
        ):
            dedup_dict[vid] = True
